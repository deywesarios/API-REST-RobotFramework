...
Conectar com a minha API
    Create Session                        ${API_BOOKS.ALIAS}            ${API_BOOKS.BASE_URL}        disable_warnings=${True}
    ${HEADERS}                            Create Directory    content-type=application/json
    Set Test Variable                     ${HEADERS}

Requisitar todos os livros
    ${RESPOSTA}                           GET On Session      ${API_BOOKS.ALIAS}       ${API_BOOKS.ENDPOINT}        
    Log                                   ${RESPOSTA.text}
...
Requisitar o livro "${ID_LIVRO}"
    ${RESPOSTA}             GET On Session                ${API_BOOKS.ALIAS}                      ${API_BOOKS.ENDPOINT}/${ID_LIVRO}
    Log                                                   ${RESPOSTA.text}
    Set Test Variable                                     ${RESPOSTA}
